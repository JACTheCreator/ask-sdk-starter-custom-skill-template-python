...
from constants.intents import THIRD_ORDER_INTENT

from utils.say import say

from utils.common import (set_next_intent, is_next_intent_error, 
                          handle_next_intent_error, get_prev_msg)

class ThirdOrderIntentHandler(AbstractRequestHandler):
    """Handler for Help Intent."""
    def can_handle(self, handler_input):
        # type: (HandlerInput) -> bool
        return is_intent_name(THIRD_ORDER_INTENT)(handler_input)

    def handle(self, handler_input):
        # type: (HandlerInput) -> Response

        # Checking if this intent should play now.
        current_intent = [THIRD_ORDER_INTENT]
